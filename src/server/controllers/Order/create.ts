...
export const create = async (
    ...
    const [orderItemArray, total_price] = await createOrderItemArray(
        inputData,
        user_id
    );
    ...
    const order_id = await OrderProvider.create({
        user_id,
        total_price,
 ...
 */
const createOrderItemArray = async (
    inputData: IBodyProps[],
    user_id: number
): Promise<[IOrderItemArray[], number]> => {
    ...
    for (const productInput of inputData) {
        ...
        if (item_data.user_id !== user_id) {
            throw new UnauthorizedError(
                "item_id: " + item_id + " inválido para esse usuário"
            );
        }
...
export const createValidation = (
    ...
    if (
        typeof bodyErrors.item_id !== "undefined" ||
        typeof bodyErrors.quantity !== "undefined"
    ) {
